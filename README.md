...
3. go to project `it.ismb.pert.cpswarm.simulationOrchestrator` 
    1. open `orchestrator.bndrun`, you have to setup the SOO by modifying the following properties' values according to the actual values and `save`
    ...
    2. open `SimulationOrchestrator.java`, uncomment the line:657 if the `opt` option is `false` for only test the stage simulation, otherwise, skip this step  
